...
class DeliverUserController extends Controller
{

    /*deliver user register*/
    ...
    public function deliverUserConfirm(Request $request)
    {
        ...
        if ($request->status == 'Confirm') {
            ...
            Alert::toast('Request is Confirmed', 'success')
                ->autoClose(1000)
                ->position('top-end');
        } else {
           ...
           $user->delete();
            Alert::toast('Request is cancelled', 'success')
                ->autoClose(1000)
                ->position('top-end');
        }
        return back();
    ...
    public function banned($id)
    {
        ...
        $user->save();
        return back()->with('success', translate('Successfully done'));;
    }
    ...
    public function ordersList(Request $request)
    {
        ...
        if ($request->search) {
            ...
            if (!is_null($request->end_date)) {
                ...
                if (empty($condition)) {
                    $orders = OrderProduct::with('orderAssignToDeliver')->where('status', 'confirmed')->whereBetween('created_at', [$start, $end])->paginate(10);
                } else {
                    $orders = OrderProduct::with('orderAssignToDeliver')->where('status', 'confirmed')->where($condition)->whereBetween('created_at', [$start, $end])->latest()->paginate(10);

                }

            } else {
                $orders = OrderProduct::with('orderAssignToDeliver')->where('status', 'confirmed')->where($condition)->latest()->paginate(10);
            }
        } else {
            $orders = OrderProduct::with('orderAssignToDeliver')->where('status', 'confirmed')->latest()->paginate(10);
        }
    ...
    public function assignStore(Request $request)
    {
        if (!$request->has('deliver_user_id')) {
            ...
            $assignUser = DeliverAssign::where('deliver_user_id', $request->deliver_user_id)
                ->where('order_id', $request->order_id)->first();

            if ($assignUser == null) {
                $assign = new DeliverAssign();
    ...
    {
        $deliver = DeliverAssign::with('deliverUser')
            ->with('orderDetails')
            ->with('deliverSummery')
            ->with('assignUser')
            ->findOrFail($id);
    ...
    public function locationStatusStore(Request $request){
        $delivermen = new DeliveymenTrack();
        ...
        $delivermen->save();
        $deliver = DeliverAssign::where('id',$request->deliver_assign_id)->first();
        $user = User::where('id',$deliver->user_id)->first();
    ...
    {
        $assignOrder = DeliverAssign::with('deliverUser')
            ->with('orderDetails')
            ->with('assignUser')
    ...
    {
        $deliverOrder = DeliverAssign::with('deliverUser')
            ->with('orderDetails')
            ->with('assignUser')
            ->where('deliver_user_id', Auth::id())
    ...
    public function deliverPickRemove($id){
        $assign = DeliverAssign::where('id',$id)->first();
        $location = DeliveymenTrack::where('deliver_assign_id',$id)->get();
    ...
    {
        $user = Auth::user();
        return view('backend.deliver.profile', compact('user'));
    }
