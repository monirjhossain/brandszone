...
class CategoryController extends Controller
{
    ...
    public function catGroupIndex(){
        $categories = Category::where('parent_category_id',0)->where('is_published', 1)->with('childrenCategories')->paginate(paginate());
        /*seller request categories*/
        $sellerRequest =Category::where('is_requested',1)->with('parent')->get();
    ...
    public function catGroupStore(Request $request){
        ...
        /** VERSION 3.2 */
        if(function_exists('write_to_lang_category_file')){
            write_to_lang_category_file($category->name);
        }
