...
class OrderController extends Controller
{
     ...
     * order_export_filter
     */
    public function order_export_filter(Request $request)
    {
        ...
        $request->validate([
            ...
            'start_date.required' => 'Please enter order from date',
            'end_date.required' => 'Please enter order to date',
        ]);


        $start_date = Carbon::parse($request->start_date)->toDateTimeString() ?? null;
        $end_date = Carbon::parse($request->end_date)->toDateTimeString() ?? null;
        $status = $request->status ?? null;
        $payment_type = $request->payment_type ?? null;
        $customer_id = $request->customer_id ?? null;
        $shop_id = $request->shop_id ?? null;
        $logistic_id = $request->logistic_id ?? null;
        $product_id = $request->product_id ?? null;

        // For testing
        // return OrderProduct::latest()
        //                     ->where('status', $status)
        ...
        if ($request != null) {
                    return Excel::download(new OrderFilterExport($start_date, $end_date, $status, $payment_type, $customer_id, $shop_id, $logistic_id, $product_id), 'orders.csv', \Maatwebsite\Excel\Excel::CSV);
        }else{
