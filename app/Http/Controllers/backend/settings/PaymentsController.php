...
class PaymentsController extends Controller
{
    ...
    public function qrcodeStore(Request $request)
    {
        ...
        overWriteEnvFile('BANK_ACCOUNT', $request->bank_account);

        if ($request->hasFile('bank_qrcode')) {
            if ($request->has('bank_qrcode')) {
                $system = Settings::where('name', 'bank_qrcode')->first();
                $system->value = fileUpload($request->bank_qrcode, 'bank_qrcode');
                $system->save();
            }
        }
        Alert::success('Done', translate('Payment\'s logo has been updated.'));
