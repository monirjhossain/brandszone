...
trait LanguageTraits
{
    ...
    //translate language save as a php file
    public function translate_categories_store(Request $request)
    {
        $language = Language::findOrFail($request->id);
        ...
        mTranslate($datas, $language->code, 'categories');
        /**VERSION 3.2::END */

        return back()->with('success',translate('Translation has been saved.'));
    }

    //languages for create product translate
    public function translate_products_create($id)
    {
        $lang = Language::findOrFail($id);

        /** VERSION 3.2 */
        if(function_exists('check_lang_file_exists')){
            check_lang_file_exists($lang->code, 'products');
        }

        return view('backend.common.setting.lang.translate_product',compact('lang'));
    }

    
    //translate language save ase a json format file
    public function translate_products_store(Request $request)
    {
        ...
        mTranslate($datas, $language->code, 'products');
        /**VERSION 3.2::END */
