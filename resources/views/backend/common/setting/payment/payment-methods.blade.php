...
@section('content')
    <div class="row">
        ...
        @csrf
            ...
            @if(env('PAYTM_MERCHANT_ID') != null &&  env('PAYTM_MERCHANT_KEY') != null &&  env('PAYTM_ACTIVE') != "NO") 
         <div class="card mt-4">
                <div class="card-header">
        ...
        @if(env('BANK_TRANSFER') == "YES") 
            <hr>
            <form method="post" action="{{route('qrcode.logo.store')}}" enctype="multipart/form-data">
                @csrf

            <div class="card mt-4">
                <div class="card-header">
                    <h2 class="card-title">@translate(Bank Payment)</h2>
                </div>
                <div class="card-body">
                    <label class="label">@translate(Account Number)</label>
                    <input type="text" placeholder="@translate(Enter bank account number)" value="{{env('BANK_ACCOUNT')}}"
                           name="bank_account"
                           class="form-control mb-2">

                    <div class="card mt-2">
                        <div class="card-header">
                            <h2 class="card-title">@translate(QR Code)</h2>
                        </div>

                    @if(getSystemSetting('bank_qrcode') != null)
                        <img src="{{filePath(getSystemSetting('bank_qrcode'))}}" class="w-25 img-fluid center m-3">
                    @endif

                    <div class="m-2 mr-4">
                        <input type="file" class="form-control" name="bank_qrcode">
                    </div>
                </div>
