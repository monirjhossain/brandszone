...
Route::get('/export/as/csv/{uuid}', function () {
    return Excel::download(new OrdersExport, 'orders.csv', \Maatwebsite\Excel\Excel::CSV);
})->name('export.as.csv');

Route::get('/export/as/xlsx/{uuid}', function () {
    return Excel::download(new OrdersExport, 'orders.xlsx', \Maatwebsite\Excel\Excel::XLSX);
})->name('export.as.xlsx');

Route::get('/export/as/xls/{uuid}', function () {
    return Excel::download(new OrdersExport, 'orders.xls', \Maatwebsite\Excel\Excel::XLS);
})->name('export.as.xls');

Route::get('/export/as/tsv/{uuid}', function () {
    return Excel::download(new OrdersExport, 'orders.tsv', \Maatwebsite\Excel\Excel::TSV);
})->name('export.as.tsv');

Route::get('/export/as/html/{uuid}', function () {
    return Excel::download(new OrdersExport, 'orders.pdf', \Maatwebsite\Excel\Excel::DOMPDF);
})->name('export.as.pdf');
...
//there are the user Manager section
    ...
    Route::get('/download/invoice/{number}/{uuid}', 'HomeController@downloadInvoice')->name('download.invoice')
        ->middleware('permissions:dashboard');
...
//site setting
    ...
    Route::get('preloader/settings', 'PreloaderController@preloaderSettings')->name('preloader.setting');//->middleware('permissions:site-setting');
    Route::post('preloader/settings/update', 'PreloaderController@preloaderUpdate')->name('preloader.update');//->middleware('permissions:site-setting');
...
//Language Setting
    Route::group(['middleware' => 'permissions:language-setup'], function () {
         ...
         * Version 3.2.0
         */
        Route::get('language/translate/categories/{id}', 'CommonController@translate_categories_create')->name('language.categories.translate'); // version 3.2
        Route::post('language/translate/categories/store', 'CommonController@translate_categories_store')->name('language.categories.translate.store')->middleware('demo'); // version 3.2

        Route::get('language/translate/products/{id}', 'CommonController@translate_products_create')->name('language.products.translate'); // version 3.2
        Route::post('language/translate/products/store', 'CommonController@translate_products_store')->name('language.products.translate.store')->middleware('demo'); // version 3.2
        /**
         * Version 3.2.0
         */
    });
...
//this route for published or unpublished
    ...
    Route::group(['middleware' => 'permissions:payment-method-setup'], function () {
        ...
        Route::post('payment/logo/qrcode/store', 'backend\settings\PaymentsController@qrcodeStore')->name('qrcode.logo.store')->middleware('demo');
    });
    ...
    if (!vendorActive()) {
        ...
        Route::group(['middleware' => 'permissions:complain-manage'], function () {
         ...
         /**
             * EXPORT ORDERS
             */
            Route::get('/order/export/index', 'backend\order\OrderController@export_index')
                    ->name('order.export.index')
                    ->middleware('demo');
                    
            Route::get('/order/export/fiter/{uuid}', 'backend\order\OrderController@order_export_filter')
                    ->name('order.export.filter')
                    ->middleware('demo');
...
/** SEARCH*/
Route::get('product/search/by/name', 'HomeController@product_search')->name('product.search.backend');

/** SEARCH*/
Route::get('user/customers', 'HomeController@users_customer')->name('users.customer');
...
/** COLOR*/
Route::get('theme/color', 'HomeController@theme_color')->name('theme.color');
Route::post('theme/color/store', 'HomeController@theme_color_store')->name('theme.color.store')->middleware('demo');
