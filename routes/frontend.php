...
Route::get('xxx', function () {
    ...
    write_to_lang_category_file('jareld');
    return 'ok';
    ...
    /**
     * MAKE A NEW INSTANCE OF THE TRANSLATE CLASS
     */


    // $cats = \DB::table('products')->get('name');

    // $datas = collect();

    // foreach ($cats as $cat) {

    //     $obj = new App\Models\Demo;
    //     $obj->key = $cat->name;
    //     $obj->value = 'name';
    //     $datas->push($obj);
    // }

    // mTranslate($datas, 'en', 'products');
...
Route::group(['middleware' => ['installed', 'HtmlMinifier']], function () {
    ...
    if (vendorActive()) {
        ...
        Route::post('/place/order/bank', 'frontend\order\OrderController@bankstore')->name('place.order.bank')->middleware('demo');
        ...
        Route::post('/place/order/bank', 'ecommerce\OrderController@bankstore')->name('place.order.bank.ecom')->middleware('demo');
